...
@section('title', __('bookmarks.bookmarks') . ' (' . __('posts.for_sale') . ')')

@section('content')
<div class="panel panel-default">
    <div class="panel-heading">{{ __('bookmarks.bookmarks') . ' (' . __('posts.for_sale') . ')' }}</div>
    ...
    <div class="panel-body">
        @if(count($bookmarks) > 0)
            ...
            {{$bookmarks->links()}}
        @else
            <p>{{ __('bookmarks.you_have_no_bookmarks_for_sale') }}</p>
        @endif
    </div>
</div>
@endsection
